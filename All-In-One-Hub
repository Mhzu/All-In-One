...
-- ACCESS CONTROL
--
-- ACCEPT   = approve this current hub session only.
-- WHITELIST = permanently allow this Roblox UserId.
-- DENY     = deny this request only.
-- BLACKLIST = permanently block this Roblox UserId.
--
-- Whitelisted users do not need to request access again.
-- Accepted (but not whitelisted) users must request again
-- when they close/reopen the hub.
--========================================================
local BOT_BASE_URL    = "https://all-in-one-approver.onrender.com"
local POLL_INTERVAL   = 3
local REQUEST_TIMEOUT = 420
...
-- New execution = new session.
local sessionId = "session-" .. tostring(os.clock()) .. "-" .. tostring(math.random(100000, 999999))
pcall(function()
...
AccessGui.Name = "AccessRequest"
AccessGui.ResetOnSpawn = false
AccessGui.IgnoreGuiInset = true
AccessGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

pcall(function()
    if syn and syn.protect_gui then
        syn.protect_gui(AccessGui)
        AccessGui.Parent = game:GetService("CoreGui")
    elseif gethui then
        AccessGui.Parent = gethui()
    else
        AccessGui.Parent = game:GetService("CoreGui")
    end
end)

if not AccessGui.Parent then
    AccessGui.Parent = lp:WaitForChild("PlayerGui")
end

local Frame = Instance.new("Frame")
Frame.Size = UDim2.new(0, 340, 0, 150)
Frame.Position = UDim2.new(0.5, -170, 0.5, -75)
Frame.BackgroundColor3 = Color3.fromRGB(28, 8, 10)
Frame.BorderSizePixel = 0
Frame.Parent = AccessGui
Instance.new("UICorner", Frame).CornerRadius = UDim.new(0, 12)

local Stroke = Instance.new("UIStroke")
Stroke.Color = Color3.fromRGB(255, 55, 55)
...
Title.Text = "Waiting for permission..."
...
CloseBtn.BackgroundColor3 = Color3.fromRGB(70, 20, 24)
...
local accessClosed = false
CloseBtn.MouseButton1Click:Connect(function()
    accessClosed = true
    if AccessGui then AccessGui:Destroy() end
end)
...
Status.TextColor3 = Color3.fromRGB(210, 150, 150)
Status.TextSize = 13
...
Dots.Size = UDim2.new(1, -24, 0, 20)
Dots.Position = UDim2.new(0, 12, 0, 115)
Dots.BackgroundTransparency = 1
Dots.Text = ""
Dots.TextColor3 = Color3.fromRGB(180, 100, 100)
Dots.TextSize = 14
...
task.spawn(function()
    local i = 0
    while AccessGui and AccessGui.Parent and not accessClosed do
        i = (i % 3) + 1
...
local function sendAccessRequest()
    ...
    if res and (res.StatusCode == 200 or res.StatusCode == 201) then
        ...
        if ok and data and data.approved == true then
            return true, false
        end
        Status.Text = "Request sent!\nWaiting for owner to Accept, Whitelist, or Deny..."
        return false, true
    end

    Status.Text = "Could not reach approval bot.\nTry again later."
    return false, false
end

local function waitForApproval()
    local startTime = os.clock()

    while (os.clock() - startTime) < REQUEST_TIMEOUT do
        if accessClosed then
            return false
        end

        local res = httpRequest({
            Url = BOT_BASE_URL
                .. "/check?userId=" .. HttpService:UrlEncode(userId)
...
do
    local immediate, waiting = sendAccessRequest()

    if immediate then
        approved = true
    elseif waiting and not accessClosed then
    ...
    if not approved then
        ...
        pcall(function()
            if syn and syn.protect_gui then
                syn.protect_gui(DenyGui)
                DenyGui.Parent = game:GetService("CoreGui")
            else
                DenyGui.Parent = game:GetService("CoreGui")
            end
        end)

        if not DenyGui.Parent then
            DenyGui.Parent = lp:WaitForChild("PlayerGui")
        end

        local DFrame = Instance.new("Frame")
        DFrame.Size = UDim2.new(0, 340, 0, 130)
        DFrame.Position = UDim2.new(0.5, -170, 0.5, -65)
        DFrame.BackgroundColor3 = Color3.fromRGB(28, 8, 10)
        DFrame.BorderSizePixel = 0
        DFrame.Parent = DenyGui
        Instance.new("UICorner", DFrame).CornerRadius = UDim.new(0, 12)

        local DStroke = Instance.new("UIStroke")
        DStroke.Color = Color3.fromRGB(220, 40, 45)
        DStroke.Thickness = 1.5
        DStroke.Parent = DFrame

        local DClose = Instance.new("TextButton")
        DClose.Size = UDim2.new(0, 28, 0, 28)
        DClose.Position = UDim2.new(1, -34, 0, 8)
        DClose.BackgroundColor3 = Color3.fromRGB(70, 20, 24)
        DClose.BorderSizePixel = 0
        DClose.Text = "X"
        DClose.TextColor3 = Color3.fromRGB(255, 180, 180)
        DClose.TextSize = 14
        DClose.Font = Enum.Font.GothamBold
        DClose.AutoButtonColor = false
        DClose.ZIndex = 10
        DClose.Parent = DFrame
        Instance.new("UICorner", DClose).CornerRadius = UDim.new(0, 6)
        ...
        DTitle.Size = UDim2.new(1, -50, 0, 30)
        DTitle.Position = UDim2.new(0, 12, 0, 14)
        DTitle.BackgroundTransparency = 1
        DTitle.Text = "Access Denied"
        ...
        DMsg.Size = UDim2.new(1, -24, 0, 60)
        DMsg.Position = UDim2.new(0, 12, 0, 50)
        DMsg.BackgroundTransparency = 1
        DMsg.Text = "Permission was not granted,\nor this account is blocked.\nRequest access again if needed."
        DMsg.TextColor3 = Color3.fromRGB(210, 150, 150)
        DMsg.TextSize = 13
...
-- =========================================================
-- APPROVED → continue loading the hub
-- =========================================================
...
function Library:CreateWindow(Settings)
    ...
    -- Prefer protected/executor GUI hosts so the hub is not cleared with PlayerGui
    ...
    function self:CreateTab(TabName)
        ...
        function Tab:CreateKeybind(Text, DefaultKey, Callback)
            ...
            Holder.MouseButton1Click:Connect(function()
                ...
                Listening = true
                KeyLabel.Text = "Press key/button..."
            end)

            UserInputService.InputBegan:Connect(function(Input, GameProcessed)
                local inputType = Input.UserInputType
                local isKeyboardOrGamepad = inputType == Enum.UserInputType.Keyboard
                ...
                if Listening then
                    if isKeyboardOrGamepad and Input.KeyCode ~= Enum.KeyCode.Unknown then
                ...
                if not GameProcessed
                    and CurrentKey
                    and CurrentKey ~= Enum.KeyCode.None
                    and isKeyboardOrGamepad
    ...
    function self:CreateGuiToggleKeybind(DefaultKey)
        ...
        UserInputService.InputBegan:Connect(function(Input, GameProcessed)
            ...
            -- Always allow binding even if the game processed the input
            if Listening then
                if isKeyboardOrGamepad and Input.KeyCode ~= Enum.KeyCode.Unknown then
                    CurrentKey = Input.KeyCode
                    Listening = false
        ...
        return {
            ...
            BeginListening = function()
                ...
                if self._GuiKeybindButton then
                    self._GuiKeybindButton.Text = "Press a key/button..."
                end
            end
        }
    end
...
-- Ensure hub is visible after build
if Window and Window.ScreenGui then
    Window.ScreenGui.Enabled = true
    pcall(function()
        print("[All-In-One Hub] Loaded — GUI should be visible")
    end)
end
...
do
    ...
    statusLabel.Text = ""
    statusLabel.TextColor3 = Color3.fromRGB(130, 220, 150)
    statusLabel.TextSize = 11
    statusLabel.Font = Enum.Font.Gotham
    statusLabel.TextXAlignment = Enum.TextXAlignment.Left
    statusLabel.LayoutOrder = 13
    statusLabel.ZIndex = 5
    statusLabel.Parent = Misc.Page
